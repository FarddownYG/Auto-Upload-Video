...
# pip install openai moviepy gtts google-api-python-client google-auth-oauthlib pillow requests
...
def pick_topic():
    ...
    prompt = """You are a viral YouTube Shorts and TikTok expert.
...
- Trending in 2025
- Under 60 characters
- Return ONLY the topic title, nothing else, no quotes."""
...
def generate_script(topic):
    ...
    prompt = f"""Write a YouTube Shorts video script in English about: "{topic}"

Rules:
- Duration: exactly 90 seconds when read aloud (about 220 words)
- Style: energetic, direct, no fluff, finance/business tone
- Structure: hook (10s) → 3 key points (60s) → call to action (20s)
...
def generate_voice(script, output_path):
    ...
    if ELEVEN_API_KEY:
        try:
            voice_id = "21m00Tcm4TlvDq8ikWAM"  # Rachel - clear English voice
            headers = {
